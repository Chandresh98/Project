...
const createReview = async (req, res) => {
  try {
    ...
    if (Object.keys(data).length == 0 || data == null) { return res.status(400).send({ Status: false, message: "Please provide the request body" }) }

    if (!validation.isValidObjectId(bookId)) { return res.status(400).send({ Status: false, message: "Please provide a valid book id" }) }
    let book = await bookModel.findById(bookId)
    if (!book) { return res.status(404).send({ Status: false, msg: "No book with the given book id was found" }) }
    if (book.isDeleted === true) { return res.status(400).send({ Status: false, message: "The requested book has been deleted" }) }
   ...
   if (!validation.isValidRating(rating)) {
      return res.status(400).send({ Status: false, message: "Please enter a rating from 1 to 5" })
    }
  reviewData['rating'] = rating
    ...
    return res.status(201).send({ Staus: true, message: "Successfuly created review", Data: reviewCount })
...
const deleteReview = async (req, res) => {
  try {
    ...
    if (!validation.isValidObjectId(bookId)) { return res.status(400).send({ status: false, message: "Please insert valid book id" }) }

    if (!validation.isValidObjectId(reviewId)) { return res.status(400).send({ status: false, message: "Please insert valid review id" }) }

    const review = await reviewModel.findById(reviewId)
    if (!review) { return res.status(404).send({ status: false, message: "No review exists with this id" }) }
    if (review.isDeleted == true) { return res.status(400).send({ status: false, message: "Review Already deleted" }) }


    const book = await bookModel.findById(bookId)

    if (!book) { return res.status(404).send({ status: false, message: "not book exists with this id" }) }
    if (book.isDeleted == true) { return res.status(400).send({ status: false, message: "This book is Deleted" }) }


    if (bookId != review.bookId) { return res.status(400).send({ Status: false, message: "The book and review dont match" }) }
